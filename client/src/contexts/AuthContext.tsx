...
type Auth = { user: Usuario | null; loading: boolean; signIn: (email: string, password: string) => Promise<void>; signInGoogle: (idToken: string) => Promise<Usuario>; signUp: (nome: string, email: string, password: string) => Promise<void>; atualizarUsuario: (usuario: Usuario) => void; signOut: () => void }
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const signInGoogle = async (idToken: string) => { const result = await api.auth.google(idToken); authStorage.set(result.token); setUser(result.user); return result.user }
