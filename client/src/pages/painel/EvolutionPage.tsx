...
import { api, type EvolutionStatus, type ModelosMensagemWhatsApp, type RegrasEnvioAutomatico } from '@/lib/api'

const QR_EXPIRACAO_SEGUNDOS = 30
const modelosPadrao: ModelosMensagemWhatsApp = { criacao: 'Ola, {{cliente}}! Seu agendamento de {{servico}} com {{profissional}} foi confirmado para {{data}} as {{hora}}.', remarcacao: 'Ola, {{cliente}}! Seu agendamento de {{servico}} foi remarcado para {{data}} as {{hora}} com {{profissional}}.', cancelamento: 'Ola, {{cliente}}! Seu agendamento de {{servico}} em {{data}} as {{hora}} foi cancelado.', atualizacao: 'Ola, {{cliente}}! Seu agendamento de {{servico}} com {{profissional}} foi atualizado: {{data}} as {{hora}}.', lembrete: 'Ola, {{cliente}}! Lembrete: seu atendimento de {{servico}} com {{profissional}} sera em {{data}} as {{hora}}.', pendente: 'Ola, {{cliente}}! Seu agendamento de {{servico}} esta pendente de confirmacao para {{data}} as {{hora}}.', confirmado: 'Ola, {{cliente}}! Seu agendamento de {{servico}} foi confirmado para {{data}} as {{hora}}.', concluido: 'Ola, {{cliente}}! Seu atendimento de {{servico}} foi concluido.', atrasado: 'Ola, {{cliente}}! Seu horario de {{servico}} esta marcado como atrasado.' }
...
export default function EvolutionPage() {
  ...
  useEffect(() => { void atualizar(); void api.evolution.mensagens().then(setModelos).catch(() => {}); void api.evolution.regrasEnvioAutomatico().then(resposta => { setEnvioAutomatico(resposta.ativo); setRegrasAutomaticas(resposta.regras) }).catch(() => {}) }, [])
  useEffect(() => {
  ...
  const salvarRegrasAutomaticas = async () => { try { setRegrasAutomaticas((await api.evolution.salvarRegrasEnvioAutomatico(regrasAutomaticas)).regras); toast.success('Regras de envio automatico salvas.'); } catch (error) { toast.error(error instanceof Error ? error.message : 'Nao foi possivel salvar as regras.') } }
  ...
  return <section className="mx-auto max-w-3xl">
    ...
    <div className="mt-6 rounded-xl border border-slate-200 bg-white p-6 shadow-sm"><h2 className="text-lg font-bold text-slate-900">Envio automatico</h2><label className="mt-3 inline-flex items-center gap-2 text-sm font-semibold text-slate-800"><input type="checkbox" checked={envioAutomatico} onChange={event => void alterarEnvioAutomatico(event.target.checked)} />Ativar envio automatico</label><p className="mt-1 text-sm text-slate-600">Desativado por padrao. Escolha abaixo quais eventos podem enviar mensagens automaticamente.</p><div className="mt-4 grid gap-2 sm:grid-cols-2">{([['criacao', 'Criacao'], ['remarcacao', 'Remarcacao'], ['cancelamento', 'Cancelamento'], ['pendente', 'Status pendente'], ['confirmado', 'Status confirmado'], ['concluido', 'Status concluido'], ['atrasado', 'Status atrasado'], ['lembrete', 'Lembrete antes do horario']] as const).map(([chave, titulo]) => <label key={chave} className="flex items-center gap-2 rounded border border-slate-200 p-3 text-sm font-medium"><input type="checkbox" disabled={!envioAutomatico} checked={regrasAutomaticas[chave]} onChange={event => setRegrasAutomaticas(atual => ({ ...atual, [chave]: event.target.checked }))} />{titulo}</label>)}</div>{regrasAutomaticas.lembrete && <label className="label mt-4">Enviar lembrete com quantos minutos de antecedencia?<input className="input-field mt-1 max-w-xs" type="number" min="5" max="10080" disabled={!envioAutomatico} value={regrasAutomaticas.antecedenciaLembreteMinutos} onChange={event => setRegrasAutomaticas(atual => ({ ...atual, antecedenciaLembreteMinutos: Number(event.target.value) }))} /></label>}<button type="button" disabled={!envioAutomatico} onClick={() => void salvarRegrasAutomaticas()} className="btn-primary mt-5">Salvar regras automaticas</button></div>
    <div className="mt-6 rounded-xl border border-slate-200 bg-white p-6 shadow-sm"><h2 className="text-lg font-bold text-slate-900">Modelos das mensagens</h2><p className="mt-1 text-sm text-slate-600">Variaveis disponiveis: cliente, servico, profissional, data e hora, entre chaves duplas.</p><div className="mt-5 grid gap-4">{([['criacao', 'Criacao'], ['remarcacao', 'Remarcacao'], ['cancelamento', 'Cancelamento'], ['lembrete', 'Lembrete'], ['pendente', 'Status: pendente'], ['confirmado', 'Status: confirmado'], ['concluido', 'Status: concluido'], ['atrasado', 'Status: atrasado'], ['atualizacao', 'Atualizacao generica']] as const).map(([chave, titulo]) => <label key={chave} className="label">{titulo}<textarea className="input-field mt-1 min-h-24" maxLength={1000} value={modelos[chave]} onChange={event => setModelos(atual => ({ ...atual, [chave]: event.target.value }))} /></label>)}</div><button type="button" onClick={() => void salvarModelos()} className="btn-primary mt-5">Salvar mensagens</button></div>
  </section>
}
