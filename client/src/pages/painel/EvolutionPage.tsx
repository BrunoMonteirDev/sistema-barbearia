...
export default function EvolutionPage() {
  ...
  return <section className="mx-auto max-w-3xl">
    ...
    <div className="mt-6 rounded-xl border border-slate-200 bg-white p-6 shadow-sm"><h2 className="text-lg font-bold text-slate-900">Modelos das mensagens</h2><p className="mt-1 text-sm text-slate-600">Variaveis disponiveis: cliente, servico, profissional, data e hora, entre chaves duplas.</p><div className="mt-5 grid gap-4">{([['criacao', 'Criacao'], ['remarcacao', 'Remarcacao'], ['cancelamento', 'Cancelamento'], ['pendente', 'Status: pendente'], ['confirmado', 'Status: confirmado / lembrete'], ['concluido', 'Status: concluido'], ['atrasado', 'Status: atrasado'], ['atualizacao', 'Atualizacao generica']] as const).map(([chave, titulo]) => <label key={chave} className="label">{titulo}<textarea className="input-field mt-1 min-h-24" maxLength={1000} value={modelos[chave]} onChange={event => setModelos(atual => ({ ...atual, [chave]: event.target.value }))} /></label>)}</div><button type="button" onClick={() => void salvarModelos()} className="btn-primary mt-5">Salvar mensagens</button></div>
  </section>
}
