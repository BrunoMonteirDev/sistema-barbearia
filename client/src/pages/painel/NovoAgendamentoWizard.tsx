...
export function NovoAgendamentoWizard({ clientes, profissionais, servicos, onClose, onCreated }: Props) {
  ...
  const avancar = () => {
    ...
    if (etapa === 3 && !servicoId) return toast.error("Escolha um servi\u00e7o para continuar.");
    if (etapa === 4 && !hora) return toast.error("Selecione um hor\u00e1rio dispon\u00edvel para continuar.");
  ...
  return <Modal title="Novo agendamento" onClose={onClose} size="wide" footer={<div className="flex w-full items-center justify-between gap-3">
    ...
    {etapa === 2 && <section><h3 className="text-xl font-bold text-slate-950">Escolha o profissional</h3><p className="mt-1 text-sm text-slate-600">Selecione quem realizar\u00e1 o atendimento.</p><div className="mt-6 grid gap-3 sm:grid-cols-2">{profissionais.filter((profissional) => profissional.ativo).map((profissional) => <Choice key={profissional.id} active={profissionalId === profissional.id} onClick={() => { setProfissionalId(profissional.id); setHora(""); }} title={profissional.nome} description={profissional.especialidade || "Profissional da barbearia"} />)}</div></section>}
