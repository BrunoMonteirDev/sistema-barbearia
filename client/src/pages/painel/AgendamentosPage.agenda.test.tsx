...
describe('agenda administrativa', () => {
  it('mostra o bloco do agendamento no horário e profissional corretos', () => {
    render(<AgendaDiaria items={[pendente]} profissionais={[profissional]} onEdit={vi.fn()} onConfirm={vi.fn()} onConclude={vi.fn()} />)
  ...
  it('executa a confirmação rápida do atendimento pendente', () => {
    ...
    render(<AgendaDiaria items={[pendente]} profissionais={[profissional]} onEdit={vi.fn()} onConfirm={onConfirm} onConclude={vi.fn()} />)
    fireEvent.click(screen.getByRole('button', { name: 'Confirmar' }))
    expect(onConfirm).toHaveBeenCalledWith(pendente)
  })

  it('exibe na semana somente os blocos da data correspondente', () => {
    render(<AgendaSemanal items={[pendente]} dataReferencia="2026-08-03" onEdit={vi.fn()} onConfirm={vi.fn()} onConclude={vi.fn()} />)
