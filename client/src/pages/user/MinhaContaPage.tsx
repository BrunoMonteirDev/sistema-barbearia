...
export function UserLayout({ children }: { children: React.ReactNode }) { const { signOut } = useAuth(); const [, go] = useLocation(); return <div className="min-h-screen bg-gray-100 md:flex"><Sidebar items={sidebarItems} onSignOut={() => { signOut(); go('/') }} title="Barbearia" /><main className="min-w-0 flex-1 p-6">{children}</main></div> }
